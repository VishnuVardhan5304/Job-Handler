...
Provide an internal **Organization Job Handler** console for data and integration teams to register, monitor, and troubleshoot **pipeline jobs** and **job problems** — not a ticketing or helpdesk system.
...
- **Job CRUD** — create, read, update, archive jobs
- **Job Problem CRUD** — log, list, update, resolve problems linked to a job
- **Dashboard** — counts by status and job type; recent jobs; unresolved problems
- **Jobs list** — search, filter by job type and status, sort, pagination
- **Four pipeline job types** — first-class enums/templates with source/target defaults
- **Job run status (simulated)** — lightweight run history with manual “simulate run”; no real orchestrator
- **PostgreSQL persistence** — Neon or compatible Postgres (aligned with sibling projects)
- **REST API** — versioned backend consumed by React UI
...
| FR-R03 | “Simulate run” action on job detail (no external system call) | SHOULD |
...
| NFR-06 | Domain terms consistent across API and UI (Job, JobProblem, JobRun — not ticket) |
